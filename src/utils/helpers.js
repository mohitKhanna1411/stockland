...
export const findConditionalOcc = (arr, key, conditionalKey, condition) => {
    let arr2 = [];

    arr.forEach((x) => {
        if (x[conditionalKey] === condition) {
            // Checking if there is any object in arr2
            // which contains the key value
            if (arr2.some((val) => { return val[key] === x[key] })) {
...
export const sanitizeData = (arr) => {
    return arr.map(item => {
        return { ...item, 'Published Date': d3.timeFormat("%d/%m/%Y")(tParser(item['Published Date'].slice(0, -5))) }
